        ...
        stage("Deployment on GCP Compute Engine"){
            steps{
                sshagent(credentials: ['GCP_DEPLOYMENT_SERVER']){
                    ...
                    sh 'ssh -o StrictHostKeyChecking=no rodolfoandresm_gmail_com@34.85.179.249 docker-compose -d -f docker_compose_m1.yml up'
                    sh 'ssh -o StrictHostKeyChecking=no rodolfoandresm_gmail_com@34.85.179.249 rm docker_compose_m1.yml'
                }
            }
